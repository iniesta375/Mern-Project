...
export default function Dashboard() {
  ...
  const handleMarkAsTaken = async (medId, time) => {
    try {
      ...
      const [adherenceData, statsData] = await Promise.all([
        api.adherence.today(),
        api.adherence.stats()
      ]);
      setAdherenceToday(adherenceData);
      ...
      console.error(err);
      showToast('Could not update medication status. Try again.', 'error');
    }
  };

  if (loading) return (
    <div className="container py-4">
      <SkeletonLine width="220px" height="36px" className="mb-2" />
      <SkeletonLine width="320px" height="18px" className="mb-4" />
      <div className="row g-4">
            ...
            {[1, 2, 3].map(i => (
              <div key={i} className="d-flex gap-3 align-items-center mb-4">
                ...
                <div className="flex-grow-1">
                  <SkeletonLine width="55%" height="16px" className="mb-2" />
                  <SkeletonLine width="40%" height="13px" />
                </div>
                <SkeletonLine width="110px" height="36px" className="rounded-3" />
              </div>
            ))}
          </SkeletonCard>
        </div>
        <div className="col-lg-4">
          <SkeletonCard>
            ...
            <SkeletonLine width="100%" height="72px" className="rounded-3 mb-3" />
            <SkeletonLine width="100%" height="72px" className="rounded-3" />
          </SkeletonCard>
        </div>
      </div>
    </div>
  ...
  const missedDoses = meds.flatMap(med =>
    med.times
      .filter(time => {
        const isTaken = adherenceToday.some(
          log => log.medication_id === med.id && log.scheduled_time.includes(time)
        );
        return !isTaken && time < currentTime;
      })
      .map(time => ({ medName: med.name, time }))
  );

  const upcomingDoses = meds.flatMap(med =>
    med.times
      .filter(time => {
        const isTaken = adherenceToday.some(
          log => log.medication_id === med.id && log.scheduled_time.includes(time)
    ...
    <div className="container py-4">
      ...
      <header className="mb-4">
        ...
        <p className="text-secondary">
          Here&apos;s your health summary for today, {format(new Date(), 'MMMM do')}.
        </p>
      </header>
                ...
                meds.flatMap(med =>
                  med.times.map(time => {
                    ...
                    return (
                      <div
                        key={`${med.id}-${time}`}
                        className="list-group-item p-3 d-flex align-items-center justify-content-between border-0 border-bottom"
                      >
                        <div className="d-flex align-items-center gap-3">
                          <div
                            className={`rounded-circle d-flex align-items-center justify-content-center ${
                              isTaken ? 'bg-success bg-opacity-10 text-success' : 'bg-light text-secondary'
                            }`}
          ...
          <div className="row g-4">
            <div className="col-md-6">
              <section className="card border-0 shadow-sm p-4 h-100">
                <h3 className="small fw-bold text-secondary text-uppercase mb-3">Adherence Rate</h3>
                <div className="d-flex align-items-end gap-3 mb-3">
                  <span className="display-5 fw-bold text-dark">{stats.percentage}%</span>
                  <div
                    className="flex-grow-1 bg-light rounded-pill overflow-hidden mb-2"
                    style={{ height: '10px' }}
                  >
                    <motion.div
                      initial={{ width: 0 }}
            ...
            <div className="col-md-6">
              <section className="card border-0 shadow-sm p-4 h-100">
                <h3 className="small fw-bold text-secondary text-uppercase mb-3">Mood Today</h3>
                <div className="d-flex align-items-center gap-3">
                  <div
                  ...
                  <div>
                    <p className="fw-bold text-dark mb-0">Log your mood</p>
                    <p className="small text-secondary mb-0">How are you feeling today?</p>
                  </div>
                </div>
                <Link
                  to="/wellness"
                  className="mt-3 small text-success fw-bold text-decoration-none d-flex align-items-center gap-1"
                >
                  Log wellness
        ...
        <div className="col-lg-4">
          ...
          <section className="card border-0 shadow-sm p-4 mb-4">
            ...
            <div className="vstack gap-3">
              {missedDoses.length === 0 && upcomingDoses.length === 0 && (
                <p className="small text-secondary text-center py-2 mb-0">
              ...
              {missedDoses.map((dose, i) => (
                <div
                  key={`missed-${i}`}
                  className="p-3 bg-warning bg-opacity-10 border border-warning border-opacity-25 rounded-3 d-flex gap-3"
                >
                  <AlertTriangle className="text-warning flex-shrink-0" size={20} />
                  <div>
                    <p className="small fw-bold text-dark mb-1">Missed Dose</p>
                    <p className="small text-secondary mb-0">
                      You missed <strong>{dose.medName}</strong> at {dose.time}.
                    </p>
                  </div>
                </div>
              ))}
              ...
              {upcomingDoses.map((dose, i) => (
                <div
                  key={`upcoming-${i}`}
                  className="p-3 bg-primary bg-opacity-10 border border-primary border-opacity-25 rounded-3 d-flex gap-3"
                >
                  <Clock className="text-primary flex-shrink-0" size={20} />
                  <div>
                    <p className="small fw-bold text-dark mb-1">Upcoming Dose</p>
                    <p className="small text-secondary mb-0">
                      <strong>{dose.medName}</strong> is due at {dose.time}.
                    </p>
                  </div>
                </div>
              ))}
